...
std::string to_stream(const Point& p) {
    return std::string{"Point{"} + to_string(p.x) + ", " + to_string(p.y) + "}";
}

std::string to_stream(const Color& c) {
    return std::string{"Color{"} + to_string(c.r) + ", " + to_string(c.g) + ", " + to_string(c.b) + "}";
}
